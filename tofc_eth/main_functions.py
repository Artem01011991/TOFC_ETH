...
def main_index():
    index_connection = index.IndexInfo(
        settings.USER_LOGIN, settings.USER_PASS, settings.USER_WMID
    )
    ...
    db_connection.set_timestamp(
        date_time_now, instrument_info["price"], "index_price_stamp"
    )
    ...
    if not buy_offer or buy_offer["notes"] < price_data[2]:
        ...
        new_average_price = (sum_price_new + sum_prior_price) / user_info["portfolio"][
            0
        ]["notes"]
    ...
    if average_price < temp:
        res = delay_func(
            index_connection.set_offer,
            user_info["portfolio"][0]["notes"],
            temp - 0.001,
            is_bid=False,
        )
    else:
    ...
    if (
        largest_prices["max"][0]
        >= instrument_info["price"]
        <= largest_prices["prior_max"][0]
    ):
        temp = None
        for i in list_off:
            if i["kind"] == 1:
        ...
        (
            db_connection.set_price_data(average_price, i["price"], i["notes"])
            for i in list_off
            if i["kind"] == 1
        )
...
def main_binance():
    binance_connection = BinanceCoreApi(
        settings.BINANCE_APIKEY, settings.BINANCE_SECRETKEY, "ETHUSDT"
    )
    ...
    db_connection.set_timestamp(
        date_time_now, symbol_info["price"], "binance_price_stamp"
    )
    ...
    if largest_prices["ids"]:
        db_connection.delete_timestamp_data(
            largest_prices["ids"], "binance_price_stamp"
        )
