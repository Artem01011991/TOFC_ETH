...
class IndexInfo:
    ...
    def _form_request(
        self, wmid, ID, date_from, date_to, count, is_anonymous, is_bid, price, offer_id
    ):
        return {
            "ApiContext": {
                "Login": self.login,
                ...
                "Signature": self._get_signature(
                    wmid, ID, date_from, date_to, offer_id
                ),
            },
            "Trading": {
                "ID": ID,
    ...
    def _get_signature(self, wmid, ID, date_from, date_to, offer_id):
        ...
        return base64.b64encode(
            hashlib.sha256(str.encode(";".join(signature_values))).digest()
        ).decode("utf-8")
    ...
    def delete_offer(self, offer_id):
        return self._get_request(
            self.urls["delete offer"], wmid=self.wmid, offer_id=str(offer_id)
        )
