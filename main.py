...
def clock_sched():
    ...
    if sell_condition and not MySqlConnection(price=sell_condition['price'], kind=0).exist():
        status = index_connection.set_offer(sell_condition['count'], sell_condition['price'], is_bid=False)

        if not status['value']['Code']:
            MySqlConnection(price=sell_condition['price'], kind=0, notes=sell_condition['count'], offerID=status['value']['OfferID']).write()

    if buy_condition and not MySqlConnection(price=buy_condition['price'], kind=1).exist():
